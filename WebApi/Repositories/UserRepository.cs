...
public class UserRepository : IUserRepository
{
    ...
    public async Task UpdatePassword(string oldpassword, string newpassword, int userid)
    {
        ...
        {
            throw new AppException("New password should be different from old password");
        }
        if (CheckChangePwd(newpassword))
        {
    ...
    public async Task UpdateUser(UserUpdateModel user, string adminlocation)
    {
        ...
        if (currentUser.Location.Equals(adminlocation))
        {
            var checkWD = CheckJoinedDateWeekDay(user.JoinDate);
            var checkJDBod = CheckJoinedDateAndDoB(user.JoinDate, user.DoB);
            var checkDob = CheckAgeIsFrom18(user.DoB);
            ...
            if (checkWD && checkJDBod && checkDob && checkJD)
            {
                ...
                {
                    currentUser.PasswordHash = BCryptNet.HashPassword(GeneratePassword(currentUser.Username, user.DoB));
                }
        ...
        else if (!currentUser.Location.Equals(adminlocation))
        {
            throw new AppException("Admin does not have permission to change Staff at diferrent location");
        }
